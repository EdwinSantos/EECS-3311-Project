...
feature -- command
	move_container(cid: STRING ; pid1: STRING ; pid2: STRING)
    	...
    	do
			...
			create move_container_oper.make(cid, pid1, pid2, model.state.get_state_msg, model.get_i, model.state.get_last_valid_i)
			if not move_container_oper.is_invalid then
				...
				create message_oper.make (model.state.get_state_msg, move_container_oper.get_error, model.get_i, model.state.get_last_valid_i)
